...
public class PanelSlide extends javax.swing.JPanel {
    ...
    public void init(Component... com) { // đưa vào danh sách các components
        if (com.length > 0) {
            for (Component c : com) {
                list.add(c);
                c.setSize(getSize());
    ...
    public void show(int index) { // hiển thị component được chọn
        if (!timer.isRunning()) {
    ...
    private void animate() { // tốc độ hoạt slide
        if (animateRight) {
